...
function LetItBeLife(){
    for(let cellToCheck = 0; cellToCheck < cellAliveToCheck.length; cellToCheck++){
        ...
        //Boucle qui fait le tour d'une cellule vivante
        for(let rowOffset = -1; rowOffset <= 1; rowOffset++){
            for(let columnOffset = -1; columnOffset <= 1; columnOffset++){
                let i = row + rowOffset;
                let j = column + columnOffset;
                let around = i * NBR_COLUMNS + j; //Around sont les cases autours de la cellule sur laquelle on boucle.

                //ne pas prendre le milieu en compte
                if(around != cellAliveToCheck[cellToCheck]){
                    if(i >= 0 && j >= 0 && i < NBR_ROWS && j < NBR_COLUMNS){ //faire attention de ne pas sortir de la grille
                        FindBornCell(around);
